          ...
          {{-- <table class="table table-striped table-hover"> --}}
            ...
            <tbody>
              ...
              <tr >
                ...
                <td>{{date('d/m/Y',$job->getActivityAppliedDate($jobseeker->id))}}</td>
                <td><form action="{{ route('jobseeker.cancel.apply', ['jobId'=>$job->id]) }}" method="POST">
                  @csrf
                  @method("delete")
                  <button type="submit" href="#" class="btn secondary-outline-btn">Cancel</button>
                </form></td>
              </tr>
              @endforeach
            </tbody>
